...
proc update_streamline_status_message {} {
	...
	if {[dui page current] == "off" || [dui page current] == "off_zoomed"} {
		...
		if {[dui page current] == "espresso" || [dui page current] == "espresso_zoomed" } {
			...
			set green_msg [subst {[translate "Dispensing:"] }]
			if {$::de1(scale_device_handle) != 0} {
				set current $::de1(scale_sensor_weight)
				set clickable_msg [subst {[seconds_text_very_abbreviated [water_pour_timer]]| [join [return_weight_measurement_grams $::de1(scale_sensor_weight) 1 1] ""]}]
			} else {
				set current [watervolume]
				set clickable_msg [subst {[seconds_text_very_abbreviated [water_pour_timer]]| [round_to_integer [watervolume]][translate "ml"]}]
			}
		...
		#		set shot_data(file_modification_date) [file mtime $path]
			...
			foreach sn {elapsed pressure weight flow flow_weight temperature_basket temperature_mix \
				...
				if {[espresso_$sn length] > 0} {
					...
					msg -WARNING "DYE::get_last noticed a zero length espresso, which it skipped"
					popup "DYE::get_last noticed a zero length espresso, which it skipped"
				}
			}
